...
const paginationStart = code.indexOf('<TablePagination', returnStart);

if (returnStart === -1 || paginationStart === -1) {
    console.error('Could not find return or pagination');
    process.exit(1);
}
        ...
        <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
          ...
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <SearchableSelect
              ...
              size="small"
              sx={{ borderRadius: '10px', fontSize: '12px', fontWeight: 600, bgcolor: '#f8fafc', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#e2e8f0' }, minWidth: 160 }}
            >
        ...
        <table style={{ borderCollapse: 'collapse', minWidth: '100%', tableLayout: 'fixed', fontFamily: 'Inter, system-ui, sans-serif', fontSize: '11px' }}>
          ...
          <tbody>
            {finalFilteredRows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((r, i) => {
                ...
                const isSelected = selectedBills ? selectedBills.includes(r._id) : false;
...
code = code.substring(0, returnStart) + newReturn + code.substring(paginationStart + 16);

// 1. Inject getColHeaderStyle
...
if (code.match(headerStyleRegex)) {
...
// Update renderRow's <tr> wrapping to apply table-row-hover and selected colors
// Because we nested it in our mapping, we don't want renderRow returning another <tr> inside `<React.Fragment><tr>...`.
// We will just remove the `<tr>` wrapper inside renderRow, BUT wait: the month header logic uses `<tr>`, so if we remove `<tr>`, we get `<tr><tr>` when it maps.
// INSTEAD, we will let renderRow return `<React.Fragment> <tr> {monthName} </tr> <tr> <td...> </tr> </React.Fragment>`, and we should NOT wrap `renderRow()` inside `<tr>` in the `<tbody>`.
// So let's fix the mapping in our newReturn:
const newMapping = `{finalFilteredRows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((r, i) => renderRow(r, page * rowsPerPage + i))}`;
code = code.replace(/\{finalFilteredRows\.slice\(page \* rowsPerPage, page \* rowsPerPage \+ rowsPerPage\)\.map\(\(r, i\) => \{[\s\S]*?return renderRow\(r, i\);[\s\S]*?\}\)\}/s, newMapping);

const renderRowTrRegex = /<tr>\n\s*\{\/\* Sl No \*\/\}/s;
code = code.replace(renderRowTrRegex, `<tr style={{
                  background: baseBg,
                  outline: false ? '2px solid rgba(124,58,237,0.4)' : 'none',
                  transition: 'background 0.2s, opacity 0.2s',
