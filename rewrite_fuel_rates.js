...
function FuelRateTab({ snackHandler }) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            snackHandler({ msg: e.response?.data?.error || 'Failed to save rate', sev: 'error' });
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <Box sx={{
            flex: 1, display: 'flex', flexDirection: { xs: 'column', lg: 'row' },
            gap: 4, px: { xs: 2, md: 5 }, py: 4, maxWidth: 1400, width: '100%', mx: 'auto'
        }}>
            {/* Left Col */}
            <Box sx={{ width: { xs: '100%', lg: 320 }, flexShrink: 0 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {PUMPS.map(pump => {
                        ...
                        return (
                            <Card key={pump} onClick={() => setSelectedPump(pump)}
                                sx={{
                                    cursor: 'pointer', borderRadius: '18px',
                                    background: isSelected ? 'linear-gradient(135deg, #1565c0 0%, #1976d2 100%)' : 'rgba(255,255,255,0.05)',
                                    border: isSelected ? '1px solid rgba(96,165,250,0.5)' : '1px solid rgba(255,255,255,0.08)',
                                    transition: 'all 0.2s ease',
                                    '&:hover': !isSelected ? { background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)' } : {},
                                }}>
                                <CardContent sx={{ p: 2.5, '&:last-child': { pb: 2.5 } }}>
                                    <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                                        <Box>
                                            <Typography variant="caption" fontWeight={800} sx={{ opacity: isSelected ? 0.9 : 0.6, letterSpacing: 0.5, color: '#fff' }}>
                                                {pump}
                                            </Typography>
                                            <Box display="flex" alignItems="baseline" gap={0.5} mt={0.5}>
                                                ...
                                                <Typography variant="h3" fontWeight={900} sx={{ color: '#fff', lineHeight: 1 }}>{latest?.rate ?? 90}</Typography>
                                                <Typography sx={{ fontSize: 14, opacity: 0.6, color: '#fff', fontWeight: 700 }}>/L</Typography>
                                            </Box>
                                        </Box>
                                        <Box sx={{ p: 1, borderRadius: '10px', bgcolor: isSelected ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.07)' }}>
                ...
                <Box sx={{ mt: 5 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                        {pumpHistory.length === 0 ? (
                            ...
                            <Box key={item._id || idx} sx={{ p: 1.5, borderRadius: '12px', bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Box>
                                    <Typography variant="body2" fontWeight={800}>₹{item.rate}</Typography>
                                    <Typography variant="caption" sx={{ opacity: 0.5 }}>{new Date(item.effectiveDate).toLocaleDateString('en-IN')}</Typography>
                                </Box>
                                {idx === 0 && <Chip label="Active" size="small" sx={{ height: 18, fontSize: '9px', fontWeight: 900, bgcolor: 'rgba(52,211,153,0.15)', color: '#34d399', border: '1px solid rgba(52,211,153,0.2)' }} />}
                            </Box>
                        ))}
                    </Box>
                </Box>
            ...
            <Box sx={{ flex: 1 }}>
                ...
                <Paper sx={{ bgcolor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', p: 4, mb: 3, backgroundImage: 'none' }}>
                    ...
                    <Box sx={{ mb: 4 }}>
                        ...
                        <TextField
                            ...
                            InputProps={{
                                ...
                                sx: { color: '#fff', fontWeight: 800, borderRadius: '16px', bgcolor: 'rgba(255,255,255,0.05)', '& input::-webkit-calendar-picker-indicator': { filter: 'invert(1)' } }
                            }}
                        />
                    ...
                    <Box sx={{ mb: 4 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: 'rgba(255,255,255,0.08)', borderRadius: '20px', border: '2px solid rgba(96,165,250,0.3)', px: 3, py: 1, '&:focus-within': { borderColor: 'rgba(96,165,250,0.7)' } }}>
                            <Typography sx={{ fontSize: 32, fontWeight: 900, opacity: 0.6, mr: 1.5 }}>₹</Typography>
                            <input
                                type="number"
                                value={rateInput}
                                onChange={e => setRateInput(e.target.value)}
                                style={{ background: 'transparent', border: 'none', outline: 'none', color: '#fff', fontSize: '64px', fontWeight: 900, width: '100%', lineHeight: 1 }}
                                placeholder="0"
                                step="0.01"
                            />
                    ...
                    <Box sx={{ p: 2, borderRadius: '16px', bgcolor: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.15)', mb: 4 }}>
...
// ─────────────────────────────────────────────────────────────────────────────
function PumpCashDiscountTab({ snackHandler }) {
    const [history, setHistory] = useState({ 'SAS-1': [], 'SAS-2': [] });
    const [selectedPump, setSelectedPump] = useState('SAS-1');
    ...
    const fetchDiscounts = async () => {
        ...
        try {
            ...
            snackHandler({ msg: 'Failed to load discounts', sev: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        const latest = history[selectedPump]?.[0];
        if (latest) setDiscountInput(String(latest.discount));
        else setDiscountInput('0');
    }, [selectedPump, history]);

    const handleSave = async () => {
        const discVal = parseFloat(discountInput);
        if (isNaN(discVal) || discVal < 0) {
            snackHandler({ msg: 'Discount must be a positive number or zero', sev: 'error' });
            return;
        }
        if (!dateInput) {
            snackHandler({ msg: 'Effective date is required', sev: 'error' });
        ...
        try {
            ...
            const res = await axios.put(\`\${API_URL}/pump-payment/cash-discounts\`,
                { pumpName: selectedPump, discount: discVal, effectiveDate: dateInput },
                { headers: { Authorization: \`Bearer \${token}\` } }
            );
            
            if (res.data.success) {
                snackHandler({ 
                    msg: \`\${selectedPump} discount updated to ₹\${discVal}/L from \${dateInput}.\`, 
                    sev: 'success' 
                });
                fetchDiscounts();
            }
        } catch (e) {
            snackHandler({ msg: e.response?.data?.error || 'Failed to save discount', sev: 'error' });
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <Box sx={{
                                ...
                                <CardContent sx={{ p: 2.5, '&:last-child': { pb: 2.5 } }}>
                                    <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                                        <Box>
                                            ...
                                            <Box display="flex" alignItems="baseline" gap={0.5} mt={0.5}>
                                                <Typography sx={{ fontSize: 13, opacity: 0.7, color: '#fff', fontWeight: 700 }}>₹</Typography>
                                                <Typography variant="h3" fontWeight={900} sx={{ color: '#fff', lineHeight: 1 }}>{latest?.discount ?? 0}</Typography>
                                                <Typography sx={{ fontSize: 14, opacity: 0.6, color: '#fff', fontWeight: 700 }}>/L</Typography>
                                            </Box>
                                        </Box>
                                        <Box sx={{ p: 1, borderRadius: '10px', bgcolor: isSelected ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.07)' }}>
                                            <AccountBalanceWalletIcon sx={{ fontSize: 20, color: isSelected ? '#fff' : 'rgba(255,255,255,0.4)' }} />
                                        </Box>
                                    </Box>
                                    {latest && (
                                        <Typography variant="caption" sx={{ opacity: 0.5, fontSize: '10px', mt: 1, display: 'block', color: '#fff' }}>
                ...
                <Box sx={{ mt: 5 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                        {pumpHistory.length === 0 ? (
                            ...
                            <Box key={item._id || idx} sx={{ p: 1.5, borderRadius: '12px', bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Box>
                                    <Typography variant="body2" fontWeight={800}>₹{item.discount}</Typography>
                                    <Typography variant="caption" sx={{ opacity: 0.5 }}>{new Date(item.effectiveDate).toLocaleDateString('en-IN')}</Typography>
                                </Box>
                                {idx === 0 && <Chip label="Active" size="small" sx={{ height: 18, fontSize: '9px', fontWeight: 900, bgcolor: 'rgba(52,211,153,0.15)', color: '#34d399', border: '1px solid rgba(52,211,153,0.2)' }} />}
                            </Box>
            ...
            <Box sx={{ flex: 1 }}>
                <Typography variant="caption" fontWeight={800} sx={{ opacity: 0.5, letterSpacing: 1, mb: 1.5, display: 'block' }}>
                    UPDATE CASH DISCOUNT SCHEDULER
                </Typography>
                <Paper sx={{ bgcolor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', p: 4, mb: 3, backgroundImage: 'none' }}>
                    <Box display="flex" alignItems="center" gap={1.5} mb={4}>
                        <Box sx={{ width: 40, height: 40, borderRadius: '12px', bgcolor: 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(16,185,129,0.2)' }}>
                            <AccountBalanceWalletIcon sx={{ color: '#10b981' }} />
                        </Box>
                        <Box>
                            <Typography variant="h6" fontWeight={900}>{selectedPump}</Typography>
                            <Typography variant="caption" sx={{ opacity: 0.5 }}>Configuring cash discount</Typography>
                        </Box>
                    </Box>
                    <Box sx={{ mb: 4 }}>
                        <Typography variant="caption" fontWeight={800} sx={{ opacity: 0.5, letterSpacing: 1, mb: 1, display: 'block' }}>
                        ...
                        <TextField
                            ...
                            onChange={e => setDateInput(e.target.value)}
                            InputProps={{
                                startAdornment: <EventIcon sx={{ color: '#10b981', mr: 1, fontSize: 20 }} />,
                                sx: { color: '#fff', fontWeight: 800, borderRadius: '16px', bgcolor: 'rgba(255,255,255,0.05)', '& input::-webkit-calendar-picker-indicator': { filter: 'invert(1)' } }
                            }}
                        />
                        <Typography variant="caption" sx={{ mt: 1, display: 'block', opacity: 0.4, fontStyle: 'italic' }}>
                            The discount will apply to all slips on or after this date.
                        </Typography>
                    </Box>
                    <Box sx={{ mb: 4 }}>
                        <Typography variant="caption" fontWeight={800} sx={{ opacity: 0.5, letterSpacing: 1, mb: 1, display: 'block' }}>
                            CASH DISCOUNT (₹ per Litre)
                        </Typography>
                        <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: 'rgba(255,255,255,0.08)', borderRadius: '20px', border: '2px solid rgba(16,185,129,0.3)', px: 3, py: 1, '&:focus-within': { borderColor: 'rgba(16,185,129,0.7)' } }}>
                            <Typography sx={{ fontSize: 32, fontWeight: 900, opacity: 0.6, mr: 1.5 }}>₹</Typography>
                            <input
                                type="number"
                                value={discountInput}
                    ...
                    <Box sx={{ p: 2, borderRadius: '16px', bgcolor: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.15)', mb: 4 }}>
                        ...
                        <Typography variant="caption" sx={{ color: '#fbbf24', opacity: 0.8, display: 'block', mt: 0.5 }}>
                            Changing the cash discount for a date will update the effective discount calculations from that date onwards.
                        </Typography>
                    </Box>
                    <Button
                        fullWidth
                        ...
                        size="large"
                        startIcon={saving ? <CircularProgress size={18} sx={{ color: '#fff' }} /> : <SaveIcon />}
                        disabled={saving || isNaN(newDiscount) || newDiscount < 0}
                        onClick={handleSave}
                        sx={{
                            py: 2, borderRadius: '18px', fontWeight: 900, fontSize: 16,
                            background: 'linear-gradient(45deg, #10b981 0%, #059669 100%)',
                            boxShadow: '0 8px 24px rgba(16,185,129,0.4)',
                            '&:hover': { boxShadow: '0 12px 32px rgba(16,185,129,0.55)', transform: 'translateY(-1px)' },
                            '&:disabled': { opacity: 0.4 },
                            transition: 'all 0.2s',
                        }}
                    >
                        {saving ? 'Scheduling Update...' : \`Apply New Discount\`}
                    </Button>
                </Paper>
            </Box>
        </Box>
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Box sx={{ flex: 1 }}>
                    <Typography variant="h6" fontWeight={900} sx={{ lineHeight: 1.2 }}>
                        Fuel Rate and Cash Discount Settings
                    </Typography>
                    <Typography variant="caption" sx={{ opacity: 0.5 }}>
                        Head Office — Pricing & Discount History Control
                    </Typography>
                </Box>
                ...
                <Tabs 
                    ...
                    sx={{
                        minHeight: 36,
                        '& .MuiTabs-indicator': { backgroundColor: tabIndex === 0 ? '#60a5fa' : '#10b981' }
                    }}
                >
                    <Tab 
                        label="Fuel Rate Settings" 
                        sx={{ 
            ...
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <TabPanel value={tabIndex} index={0}>
                    <FuelRateTab snackHandler={setSnack} />
                </TabPanel>
                <TabPanel value={tabIndex} index={1}>
                    <PumpCashDiscountTab snackHandler={setSnack} />
                </TabPanel>
            </Box>
...
fs.writeFileSync('frontend/review-dashboard/UI2/src/pages/FuelRateSettings.jsx', code);
console.log('FuelRateSettings updated');
